...
def get_mech_info(exp_set, calc_type, x_source, conds_source, gases):
    ...
    assert x_source in ('plot', 'exps') and conds_source in ('plot', 'exps'), (
        "x_source and conds_source should be 'plot' or 'exps'")
    ...
    if meas_type in ('abs', 'emis'):
        ...
        x_titles = (cond_titles, target_titles)
        xdata = conds
    ...
    if calc_type in ('sens', 'rop'):
        ...
        max_num_rxns = 0
        for gas in gases:
            if gas.n_reactions > max_num_rxns:
...
def check_sources(x_source, conds_source):
...
def create_mech_opts_lst(mech_names):
